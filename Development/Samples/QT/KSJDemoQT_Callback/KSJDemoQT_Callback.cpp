...
KSJDemoQT_Callback::KSJDemoQT_Callback(QWidget *parent) :
    QDialog(parent),
	m_bPreview(false), 
	m_bCallback(false),
	m_bCallbackEx(false),
    ui(new Ui::KSJDemoQT_Callback)
{
    ui->setupUi(this);
...
KSJDemoQT_Callback::~KSJDemoQT_Callback()
{
	KSJ_UnInit();
    delete ui;
}
...
void KSJDemoQT_Callback::on_DEVICE_LIST_ComboBox_currentIndexChanged(int index)
{
	m_nDeviceCurSel = index;
	UpdateInterface();
}

void KSJDemoQT_Callback::GetRealExposureTime()
{
...
void KSJDemoQT_Callback::on_ExposureTimeSpinBox_valueChanged(int arg1)
{
	KSJ_SetParam(m_nDeviceCurSel, KSJ_EXPOSURE, arg1);
	GetRealExposureTime();
}

void KSJDemoQT_Callback::on_ExposureLinesSpinBox_valueChanged(int arg1)
{
	KSJ_SetParam(m_nDeviceCurSel, KSJ_EXPOSURE_LINES, arg1);
	GetRealExposureTime();
}

void KSJDemoQT_Callback::on_GainSpinBox_valueChanged(int arg1)
{
...
void KSJDemoQT_Callback::on_SetPreviewFovButton_clicked()
{
	int nColStart;
	int nRowStart;
	int nColSize;
...
void KSJDemoQT_Callback::on_PreviewButton_clicked()
{
...
{
	KSJDemoQT_Callback *pVCDemoDlg = (KSJDemoQT_Callback *)lpContext;
...
void KSJDemoQT_Callback::PreviewCallback(unsigned char *pImageData, int nWidth, int nHeight, int nBitCount)
{
	DrawCrossLine(1275, 1023, 10, pImageData, nWidth, nHeight, nBitCount);
}

void KSJDemoQT_Callback::DrawCrossLine(unsigned int nX, unsigned int nY, unsigned int nLineWidth, unsigned char *pImageData, int nWidth, int nHeight, int nBitCount)
{
...
void KSJDemoQT_Callback::PreviewCallbackEx(HDC  hDC, int nWidth, int nHeight, int nBitCount)
{
	DrawSystemTime(hDC, nWidth, nHeight);
}

void KSJDemoQT_Callback::DrawSystemTime(HDC  hDC, int nWidth, int nHeight)
{
	QString strBuffer;
	QDateTime time;
	...
	TextOut(hDC, nWidth / 20, nHeight / 14, strBuffer.toLatin1().data(), strBuffer.length());
	::SelectObject(hDC, hfOld);
	::DeleteObject(hf);
}

void KSJDemoQT_Callback::on_SetCallbackButton_clicked()
{
	QPalette pal = ui->SetCallbackButton->palette();
	if (m_bCallback)
	{
		m_bCallback = false;
		pal.setColor(QPalette::Button, m_col);
	...
	ui->SetCallbackButton->setPalette(pal);
	ui->SetCallbackButton->setAutoFillBackground(true);
...
void KSJDemoQT_Callback::on_SetCallbackExButton_clicked()
{
	QPalette pal = ui->SetCallbackExButton->palette();
	if (m_bCallbackEx)
	{
		m_bCallbackEx = false;
		pal.setColor(QPalette::Button, m_col);
	...
	ui->SetCallbackExButton->setPalette(pal);
	ui->SetCallbackExButton->setAutoFillBackground(true);
...
void KSJDemoQT_Callback::on_SetCaptureFovButton_clicked()
{
	int nColStart;
	int nRowStart;
	int nColSize;
...
void KSJDemoQT_Callback::ConvetData(unsigned char *pData, int nWidth, int nHeight, int nBitCount, unsigned char* pConvertData)
{
	int nPixelBytes = nBitCount >> 3;
	int nWidthBytes = nWidth * nPixelBytes;
...
void KSJDemoQT_Callback::on_CaptureButton_clicked()
{
	if (m_nDeviceCurSel == -1)   return;
...
void KSJDemoQT_Callback::UpdateInterfaceFunction()
{
...
void KSJDemoQT_Callback::UpdateInterface()
{
	if (m_nDeviceCurSel == -1)   return;
...
void KSJDemoQT_Callback::UpdateDeviceList()
{
...
void KSJDemoQT_Callback::ShowErrorInfo(int nRet)
{
