...
void CKSJDemoVCDlg::Capture()
{
	...
	if (bCheck)
	{
		...
		if (bCheck)
		{
			_stprintf_s(szFileName, _T("capture-%04d-%02d-%02d-%02d-%02d-%02d-%03d-%05d.Tiff"), LocalTime.wYear, LocalTime.wMonth, LocalTime.wDay, LocalTime.wHour, LocalTime.wMinute, LocalTime.wSecond, LocalTime.wMilliseconds, (int)fElapse);
...
//-----------------------------------------------------------------------------
// 函数：OnBnClickedCheck16bit
// 功能：设置相机的16位模式
// 参数：无
// 返回：无
// 说明：在界面选择16bit选择框触发函数
//-----------------------------------------------------------------------------
