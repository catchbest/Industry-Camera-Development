...
void CKSJDemoVCDlg::OnBnClickedButtonReadRom()
{
	...
	ulRomAddress = (unsigned long)_tcstoll(szText, NULL, 16);

	if (ulRomAddress<g_RomRange[Type].ulMin || ulRomAddress>g_RomRange[Type].ulMax)
	{
		MessageBox(_T("RomAddress out of range"));
		return;
	}
	...
	_stprintf_s(szText, _T("%X"), btValue);
...
void CKSJDemoVCDlg::OnBnClickedButtonWriteRom()
{
	...
	btValue = (unsigned char)_tcstoll(szText, NULL, 16);
...
void CKSJDemoVCDlg::UpdateInterfaceUserZone()
{
	...
	TCHAR   szText[64] = { '\0' };
	_stprintf_s(szText, _T("%X-%X"), g_RomRange[Type].ulMin, g_RomRange[Type].ulMax);
	((CStatic*)GetDlgItem(IDC_STATIC_EEPROM_RANGE))->SetWindowText(szText);
	_stprintf_s(szText, _T("%X"), g_RomRange[Type].ulMin);
