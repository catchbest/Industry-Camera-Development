...
void CKSJDemoVCDlg::UpdateInterface()
{
	...
	KSJ_GetParamRange(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE, &nMin, &nMax);
	KSJ_GetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE, &nCur);
	...
	KSJ_GetParamRange(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE_LINES, &nMin, &nMax);
	KSJ_GetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE_LINES, &nCur);
	...
	KSJ_GetParamRange(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_RED, &nMin, &nMax);
	KSJ_GetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_RED, &nCur);
	...
	KSJ_PreviewGetDefaultFieldOfView(m_DeviceInfo[m_nDeviceCurSel].nIndex, &nColStart, &nRowStart, &nColSize, &nRowSize, &ColAddressMode, &RowAddressMode);
	...
	KSJ_CaptureGetDefaultFieldOfView(m_DeviceInfo[m_nDeviceCurSel].nIndex, &nColStart, &nRowStart, &nColSize, &nRowSize, &ColAddressMode, &RowAddressMode);
	...
	KSJ_PreviewGetStatus(m_DeviceInfo[m_nDeviceCurSel].nIndex, &PreviewStatus);
...
void CKSJDemoVCDlg::UpdateDeviceList(void)
{
	...
	DEVICEINFO temp;//根据serials号升序排序
	int i,j;
	for (i = 0; i < m_nDeviceNum - 1; i++)
	{
		for (j = 0; j<m_nDeviceNum - 1 - i; j++)
		{
			if (m_DeviceInfo[j].nSerials > m_DeviceInfo[j+1].nSerials)
			{
				temp = m_DeviceInfo[j];
				m_DeviceInfo[j] = m_DeviceInfo[j + 1];
				m_DeviceInfo[j + 1] = temp;
			}
		}
	}
...
void CKSJDemoVCDlg::GetRealExposureTime()
{
	...
	KSJ_ExposureTimeGet(m_DeviceInfo[m_nDeviceCurSel].nIndex, &fExposureTimeMs);
...
void CKSJDemoVCDlg::OnEnChangeEditExposureTimeMs()
{
	...
	KSJ_SetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE, m_dwExposureTimeMs);
...
void CKSJDemoVCDlg::OnEnChangeEditGain()
{
	...
	KSJ_SetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_RED, nGain);
	KSJ_SetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_GREEN, nGain);
	KSJ_SetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_BLUE, nGain);
}
...
void CKSJDemoVCDlg::OnEnChangeEditExposureLines()
{
	...
	KSJ_SetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE_LINES, m_dwExposureLines);
...
void CKSJDemoVCDlg::OnBnClickedButtonPreviewFovSet()
{
	...
	KSJ_PreviewSetFieldOfView(m_DeviceInfo[m_nDeviceCurSel].nIndex, nColStart, nRowStart, nColSize, nRowSize, ColAddressMode, RowAddressMode);
	
	// User may not set FOV correctly, ksjapi will correct it!
	KSJ_PreviewGetFieldOfView(m_DeviceInfo[m_nDeviceCurSel].nIndex, &nColStart, &nRowStart, &nColSize, &nRowSize, &ColAddressMode, &RowAddressMode);
	SetDlgItemInt(IDC_EDIT_PREVIEW_COL_START, nColStart);
	SetDlgItemInt(IDC_EDIT_PREIVEW_ROW_START, nRowStart);
	SetDlgItemInt(IDC_EDIT_PREIVEW_COL_SIZE, nColSize);
	SetDlgItemInt(IDC_EDIT_PREIVEW_ROW_SIZE, nRowSize);
...
void CKSJDemoVCDlg::OnBnClickedButtonCaptureFovSet()
{
	...
	KSJ_CaptureSetFieldOfView(m_DeviceInfo[m_nDeviceCurSel].nIndex, nColStart, nRowStart, nColSize, nRowSize, ColAddressMode, RowAddressMode);

	KSJ_CaptureGetFieldOfView(m_DeviceInfo[m_nDeviceCurSel].nIndex, &nColStart, &nRowStart, &nColSize, &nRowSize, &ColAddressMode, &RowAddressMode);
	SetDlgItemInt(IDC_EDIT_CAPTURE_COL_START, nColStart);
	SetDlgItemInt(IDC_EDIT_CAPTURE_ROW_START, nRowStart);
	SetDlgItemInt(IDC_EDIT_CAPTURE_COL_SIZE, nColSize);
	SetDlgItemInt(IDC_EDIT_CAPTURE_ROW_SIZE, nRowSize);
...
void CKSJDemoVCDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (nIDEvent == TIMERID_GET_FRAME_RATE)
	{
		...
		KSJ_PreviewGetFrameRate(m_DeviceInfo[m_nDeviceCurSel].nIndex, &fFrameRate);
...
void CKSJDemoVCDlg::OnBnClickedCheckPreviewstart()
{
	...
	KSJ_PreviewSetPos(m_DeviceInfo[m_nDeviceCurSel].nIndex, hPreviewWnd, 0, 0, nPreviewWndWidth, nPreviewWndHeight);

	BOOL bCheck = ((CButton*)GetDlgItem(IDC_CHECK_PREVIEWSTART))->GetCheck();

	int nRet = KSJ_PreviewStart(m_DeviceInfo[m_nDeviceCurSel].nIndex, (bCheck ? true : false));
...
afx_msg LRESULT CKSJDemoVCDlg::MsgCaptureStart(WPARAM wParam, LPARAM lParam)
{
	...
	KSJ_GetParam(m_DeviceInfo[m_nDeviceCurSel].nIndex, KSJ_EXPOSURE, &nExposureTimeMs);
...
void CKSJDemoVCDlg::Capture()
{
	...
	int nRet = KSJ_CaptureGetSizeExEx(m_DeviceInfo[m_nDeviceCurSel].nIndex, &nCaptureWidth, &nCaptureHeight, &nCaptureBitCount, &nBitsPerSample);
	ShowErrorInfo(nRet);
	...
	nRet = KSJ_CaptureRgbData(m_DeviceInfo[m_nDeviceCurSel].nIndex, pImageData);
	ShowErrorInfo(nRet);
	if (nRet != RET_SUCCESS)
	{
...
void CKSJDemoVCDlg::UpdateInterfaceFunction()
{
	...
	do{
		...
		if (nIndex < 0) return;
		int nSupport = 0;
		KSJ_QueryFunction(m_DeviceInfo[m_nDeviceCurSel].nIndex, (KSJ_FUNCTION)i, &nSupport);
