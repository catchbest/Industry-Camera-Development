...
void CKSJDemoVCDlg::UpdateDeviceList(void)
{
	...
	for (int i = 0; i<m_nDeviceNum; i++)
	{
		...
		if (nRet == RET_SUCCESS)
		{
			_stprintf_s(szMenuItem, _T("Index(%d)-Type(%s)-Serials(%d)-FwVer(%d.%d)-FpgaVer(%d.%d)-%s"), i, g_szDeviceType[m_DeviceInfo[i].DeviceType], m_DeviceInfo[i].nSerials, btMajVersion, btMinVersion, btFpgaMajVersion, btFpgaMinVersion, _T("Has Opened"));
		}
		else
		{
			_stprintf_s(szMenuItem, _T("Index(%d)-%s"), i, _T("Opened By Other Process, You Should Close it First in Other Process"));
		}
...
void CKSJDemoVCDlg::UpdateInterfaceMultiProcess()
{
	...
	_stprintf_s(szOpenStatus, Status == KSJ_DS_CLOSE ? _T("Has Closed In Current Process.") : _T("Has Open In Current Process."));
